...
- **⬅️➡️ / H L / A D**：左右移動板子
- **Space**：加速擊打 (Slam) / 發射雷射砲（裝備雷射道具時）

#### 🎁 道具系統 (Power-Ups)

在打磚塊模式中擊碎磚塊時，有機率掉落膠囊道具，板子接住即可觸發豐富強化效果：

| 代號 | 道具名稱 | 類型 | 效果說明 |
| :---: | :--- | :---: | :--- |
| **`L`** | **加長板子 (Long Paddle)** | 累加時效 | 板子長度累加，可連續疊加至填滿畫面 (MAX)！ |
| **`M`** | **多球分裂 (Multi Ball)** | 即時生效 | 場上每顆球分裂出 2 顆新球，多球並進大幅加快清磚效率 |
| **`P`** | **穿透破壞 (Pierce Ball)** | 持續 8s | 球體直接穿透磚塊不反彈，外圈散發玫瑰色破壞光環 |
| **`S`** | **安全護網 (Safety Net)** | 單次消耗 | 底線生成一道天藍色防護能量線，防止漏球致命一次 |
| **`C`** | **同色爆破 (Color Bomb)**  | 持續 10s | 擊中磚塊時，連鎖引爆相鄰所有同色的方塊 |
