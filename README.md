...
- **⬅️➡️ / H L / A D**：左右移動板子
- **按住 Space**：**洛克人蓄力集氣 (Hold to Charge)**
  - 按住空白鍵 0.9 秒，板子四周環繞順時針跑馬燈流光發光。
- **放開 Space 瞬間揮拍**：**釋放 3x3 蓄力引爆彈 (Charged 3x3 Blast)**
  - 在放開瞬間（0.25 秒窗口內）擊球，賦予球體 +15% 加速並灌注引爆能量。
  - 球體散發莫蘭迪琥珀金高能脈衝光芒，擊中磚塊時以該點為中心引爆 3x3 九宮格大範圍磚塊並自然反彈！
- **輕按 Space**：一般揮拍加速 (+15%) / 發射雷射砲（裝備雷射道具時）

#### 🎁 道具系統 (Power-Ups)

在打磚塊模式中以普通球擊碎磚塊時，有 10% 機率掉落膠囊道具，板子接住即可觸發豐富強化效果：

| 代號 | 道具名稱 | 類型 | 效果說明 |
| :---: | :--- | :---: | :--- |
| **`L`** | **加長板子 (Long Paddle)** | 累加時效 | 板子長度累加，可連續疊加至填滿畫面 (MAX)！ |
| **`M`** | **多球分裂 (Multi Ball)** | 即時生效 | 場上每顆球分裂出 2 顆新球，多球並進加快清磚效率 |
| **`P`** | **穿透破壞 (Pierce Ball)** | 持續 8s | 球體直接穿透磚塊不反彈，外圈散發玫瑰色破壞光環 |
| **`S`** | **安全護網 (Safety Net)** | 單次消耗 | 底線生成一道天藍色防護能量線，防止漏球致命一次 |
| **`A`** | **雷射火砲 (Laser Gun)**   | 持續 12s | 板子兩側裝備砲台，按下 `Space` 連射光束向上消滅磚塊 |
